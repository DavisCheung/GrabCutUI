...
def upload_file():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            output_img.save(os.path.join(app.config["UPLOAD_FOLDER"], filename_out), "png")
...
def preview():
    ...
    if exists(os.path.join(app.config["UPLOAD_FOLDER"], INPUT_NAME)):
        ...
        return render_template("preview.html", imagename=INPUT_NAME, outname=OUTPUT_NAME, imgwidth=img_width, imgheight=img_height)
    else:
